...
export function Home({ onStart }: HomeProps) {
    return (
        <div className="min-h-screen bg-white font-sans overflow-y-auto" dir="rtl">
            {/* Header */}
            <header className="border-b border-slate-100 sticky top-0 bg-white/80 backdrop-blur-md z-50">
                <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
                    ...
                    <button onClick={onStart} className="bg-indigo-50 text-indigo-600 font-semibold px-6 py-2.5 rounded-full hover:bg-indigo-100 transition-colors">
                        התחברות
                    </button>
                </div>
            </header>
            ...
            <section className="py-24 px-6 relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    ...
                    <h1 className="text-5xl md:text-7xl font-extrabold text-slate-900 tracking-tight mb-8 leading-tight">
                        משפת אינטרנט <br/> לפיתוח <span className="text-transparent bg-clip-text bg-gradient-to-l from-indigo-600 to-cyan-500">Full-Stack</span> מודרני
                    </h1>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                        <button onClick={onStart} className="bg-indigo-600 text-white font-medium text-lg px-8 py-4 rounded-full shadow-xl shadow-indigo-200 hover:shadow-2xl hover:-translate-y-1 hover:bg-indigo-700 transition-all flex items-center gap-2">
                            מתחילים ללמוד עכשיו
                            <ChevronLeft className="w-5 h-5" />
                        </button>
                        <a href="#syllabus" className="bg-white text-slate-700 border border-slate-200 font-medium text-lg px-8 py-4 rounded-full hover:bg-slate-50 transition-all">
                            צפייה בסילבוס
                        </a>
                    </div>
                </div>
            ...
            <section className="py-16 bg-slate-50 border-y border-slate-100">
                <div className="max-w-3xl mx-auto px-6">
                    <div className="bg-white border border-yellow-200 rounded-2xl p-8 flex gap-6 shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-slate-600 leading-relaxed mb-4">
                                מערכת זו נועדה למטרות למידה והדגמה בלבד. **אין להזין לה פרטי אשראי אמיתיים או מידע מזהה אישי.** ההרשמה כרגע פתוחה בחינם כחלק מההדגמה (Demo). כאשר הקורס יושק רשמית, מערכת התשלומים תופעל מול חברות אשראי מאובטחות.
                            </p>
                            <button onClick={onStart} className="text-yellow-700 font-semibold hover:text-yellow-800 underline underline-offset-4">
                                הבנתי, הייתי רוצה להמשיך להדגמה
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="grid md:grid-cols-3 gap-12">
                        {[
                            ...
                            { title: 'מנטור AI צמוד', desc: 'נתקעתם? לא הבנתם? רובוט אינטליגנטי זמין בכל עת בלחיצת כפתור בסביבת הלמידה.' }
                        ].map((b, i) => (
                            <div key={i} className="text-center">
                                <div className="w-16 h-16 bg-indigo-50 rounded-2xl flex items-center justify-center mx-auto mb-6">
                                    <CheckCircle2 className="w-8 h-8 text-indigo-600" />
                                </div>
                                <h3 className="text-2xl font-bold text-slate-800 mb-4">{b.title}</h3>
